...
def dfs(problem: SearchProblem):
    ...
    """
    fringe = util.LIFO()
    fringe.put((problem.get_start_state(), []))
    visited = set()

    while not fringe.is_empty():
        state, actions = fringe.get()
        if state in visited:
            continue
        visited.add(state)
        ...
        for successor, action, _ in problem.get_successors(state):
            if successor not in visited:
                fringe.put((successor, actions + [action]))
    return []
...
def ucs(problem: SearchProblem):
    ...
    fringe.put((problem.get_start_state(), []), 0)
    visited = {}
    ...
    while not fringe.is_empty():
        ...
        for successor, action, step_cost in problem.get_successors(state):
            if successor not in visited:
                ...
                cost = problem.total_action_cost(new_actions)
                fringe.put((successor, new_actions), cost)
    return []
...
def astar(problem: SearchProblem, heuristic=null_heuristic):
    ...
    fringe.put((start, []), heuristic(start, problem))
    visited = {}
    ...
    while not fringe.is_empty():
        ...
        for successor, action, _ in problem.get_successors(state):
            if successor not in visited:
                new_actions = actions + [action]
                g = problem.total_action_cost(new_actions)
                h = heuristic(successor, problem)
                fringe.put((successor, new_actions), g + h)
    return []
